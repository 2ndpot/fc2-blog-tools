        ...
        if (path.length >= 4 && path.length % 2 === 0 && lastLink === "strong") {
          ...
          if (targetCells.length > 0) {
            ...
            return {
              ...
              logMsg: `🔗 X-Chain 数字[${num}: R${startCell.row+1}C${startCell.col+1} から R${endCell.row+1}C${endCell.col+1} への連鎖により、${targetCells.map(c=>`R${c.row+1}C${c.col+1}`).join(', ')} の[${num}]を除外できます。`,
              logClass: "chain"
            };
          }
        }
